...
//$this->registerJsFile('/web/js/script.jquery.js', ['position' => \yii\web\View::POS_READY]);
$script = <<< JS
    initAce("socialsform-code");
JS;
$script1 = '
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            if (data.success) {
                ...
                $("#errorCt").find("pre").text(data.result);
                $("#errorCt").show();
            }
        },
...
<div class="form">
    ...
    <?php $form = ActiveForm::begin(['options' => ['class' => 'edit-form', 'id'=>'socialsForm']]); ?>
...
<div class="row">
    ...
    <?= $form->field($model, 'client_id')->textInput(['maxlength' => 255], ['class' => 'input-modal']) ?>
</div>
    ...
    <?php echo $form->error($model,'start_url');*/ ?>
        <?= $form->field($model, 'client_secret')->textInput(['maxlength' => 255], ['class' => 'input-modal']) ?>
    </div>
    ...
    <?php echo $form->error($model,'start_url');*/ ?>
        <?= $form->field($model, 'application_key')->textInput(['maxlength' => 255], ['class' => 'input-modal']) ?>
    </div>
    ...
    <?php echo $form->error($model,'start_url');*/ ?>
        <?= $form->field($model, 'redirect_uri')->textInput(['maxlength' => 255], ['class' => 'input-modal']) ?>
    </div>
    ...
    <?php echo $form->error($model,'start_url');*/ ?>
        <?= $form->field($model, 'access_token')->textInput(['maxlength' => 255], ['class' => 'input-modal']) ?>
    </div>
    ...
    <?php echo $form->error($model,'start_url');*/ ?>
        <? //$form->field($model, 'secret_session_key')->textInput(['maxlength' => 255], ['class' => 'input-modal']) ?>
    </div>
...
<fieldset id="errorCt" style="border:1px solid red;display:none;">
    <legend style="padding: 0 5px;">Ошибка:</legend>
    <pre><?=Yii::$app->session->getFlash('error')?></pre>
</fieldset>

<fieldset id="resultCt" style="border:1px solid black;display:none;">
