...
create_UUID(){let dt=new Date().getTime();let uuid='gxxxxxxxx'.replace(/[xy]/g,function(c){const r=(dt+Math.random()*16)%16|0;dt=Math.floor(dt / 16);return(c=='x'?r:(r&0x3|0x8)).toString(16);});return uuid;}};;class Auth{constructor(page){this.page=page;this.userData={};this.token='';this.formIdn='';switch(this.page){case'login':this.formIdn='mylsAuthForm';break;case'registration':this.formIdn='mylsRegistrationForm';break;case'restore':this.formIdn='mylsRestoreForm';this.token=app.getUrlParameter('token');if(this.token){$('#form-container').attr('action','/site/restore?token='+this.token);}
break;}}
...
let funcPromice=[app.translate.getTranslate(),app.getSettings()];if(this.page=='registration'){let token=app.getUrlParameter('token');if(!token){window.location.href='/site/login';}
...
getLinkAuth(){return{colSpan:2,dataField:"auth",label:{visible:false},editorOptions:{text:app.translate.saveString("Авторизация"),},template:function(data){return'<a href="/site/login" title="'+app.translate.saveString("Войти")+'">'+app.translate.saveString("Войти")+'</a>';}}}
...
getLinkRestore(){return{colSpan:2,dataField:"restore",label:{visible:false},template:function(data){return'<a href="/site/restore" title="'+app.translate.saveString("Восстановить пароль")+'">'+app.translate.saveString("Забыли пароль?")+'</a>';}}}
