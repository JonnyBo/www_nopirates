...
$resSQL = $this->selectAll("select a.account_num,
       ...
       t.territory as city,
       iif(cl.sys_type_person_id = 2, cl.inn, cd1.document) as inn,
       iif(p.amount_of_payment < amount_services_account(a.account_id), p.amount_of_payment, amount_services_account(a.account_id)) as sum_account,
       a.payer_id,
       cl.sys_type_person_id,
       p.invoice_num,
       lpad(extract(day from p.invoice_date), 2, '0') || '/' || lpad(extract(month from p.invoice_date), 2, '0') || '/' || lpad(extract(year from p.invoice_date), 4, '0') as invoice_date
from payments p
   inner join accounts a on a.account_id = p.account_id
   inner join clients c on c.client_id = a.client_id
   left outer join client_per_document cd on cd.client_id = c.client_id
     and cd.is_main = 1
   left outer join clients cl on (cl.client_id = a.payer_id)
   left outer join client_per_document cd1 on cd1.client_id = cl.client_id
     and cd1.is_main = 1
   left outer join sys_countries s on s.sys_country_id = cl.sys_country_id
   left outer join territories t on t.t_id = cl.city_id
where p.payment_id = :id  ", $params, $this->ddb);
...
$this->outputFilename = 'factura_' . str_replace('/', '-', $data['invoice_num']) . ' ' . $data['client_name'] . '.pdf';

echo '<main>
        <h1>Factura&nbsp;<span class="cyan">' . $data['invoice_num'] . '</span></h1>
        <h2>Proforma&nbsp;<span  class="cyan">' . $data['account_num'] . '</span></h2>
        <hr />
        <table style="width: 100%;" class="main-info">
            <tbody>
                <tr>
                    <td style="width: 50%; vertical-align: top;padding-left:16px;">Fecha
                        <br /><strong>' . $data['invoice_date'] . '</strong></td>
...
$dataSQL = $this->selectAll("select iif(p1.amount_of_payment < s.amount_with_tax, p1.amount_of_payment, s.unit_price) as unit_price,
       iif(p1.amount_of_payment < s.amount_with_tax, 1, s.quantity) as quantity,
       iif(p1.amount_of_payment < s.amount_with_tax, null, cast(iif(s.sys_rate_type_id = 2, s.rate_discount, s.amount_discount) as int)) as amount_discount,
       iif(s.sys_rate_type_id = 2, '%', '&euro;') discount_symbol,
       iif(p1.amount_of_payment < s.amount_with_tax, p1.amount_of_payment, s.amount_with_tax) as amount_with_tax,
       s.comment,
       ...
       p1.purpose_payment 
from payments p1
   inner join services s on s.account_id = p1.account_id
   inner join accounts a on a.account_id = p1.account_id
...
where p1.payment_id = :id", $params, $this->ddb);
...
foreach ($prices as $price) {
    ...
    if ($price['purpose_payment'])
        echo '   <br />' . $price['purpose_payment'];
